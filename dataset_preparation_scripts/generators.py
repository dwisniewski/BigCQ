...
class CQGenerator:
    def __init__(self, spo_templates_path: str, subclass_templates_path: str, synonymes_path: str):
    ...
    def materialize_to_both_spo_subclass(self, analyzed_shape: Dict[str, Any],
        ...
        result += self.materialize_placeholders_with_phrases(
            ...
            SynonymesGenerator(
                self.spo_templates[question_type], self.synonymes).get_all_expansions())

        result += self.materialize_placeholders_with_phrases(
            analyzed_shape, placeholders_without_verb,
            SynonymesGenerator(
                self.subclass_templates[question_type], self.synonymes).get_all_expansions(),
    ...
    def make_cqs(self, verbalization: str, analyzed_shape: Dict[str, Any]):
        ...
        if analyzed_shape['VERB'] is not None:
            ...
            queries['ASK'] = self.materialize_to_both_spo_subclass(analyzed_shape, {'CAR', 'VERB', 'CAD'}, 'ASK')

            if not analyzed_shape['complex_domain'] and len(analyzed_shape['domain_elems']) > 0:
                for query_type in ['SELECT_CAD', 'SELECT_COUNT_CAD']:
                    queries[query_type] = self.materialize_to_both_spo_subclass(analyzed_shape, {'CAR', 'VERB'}, query_type)
            if not analyzed_shape['complex_range'] and len(analyzed_shape['range_elems']) > 0:
                for query_type in ['SELECT_CAR', 'SELECT_COUNT_CAR']:
                    queries[query_type] = self.materialize_placeholders_with_phrases(analyzed_shape, {'CAD', 'VERB'}, SynonymesGenerator(self.spo_templates[query_type], self.synonymes).get_all_expansions())

            for query_type in ['SELECT_VERB', 'SELECT_COUNT_VERB']:
                queries[query_type] = self.materialize_placeholders_with_phrases(analyzed_shape, {'CAD', 'CAR'}, SynonymesGenerator(self.spo_templates[query_type], self.synonymes).get_all_expansions())
        else:
            queries['ASK'] = self.materialize_placeholders_with_phrases(analyzed_shape, {'CAD', 'CAR'}, SynonymesGenerator(self.subclass_templates['ASK'], self.synonymes).get_all_expansions())

            if not analyzed_shape['complex_domain'] and len(analyzed_shape['domain_elems']) > 0:
                for query_type in ['SELECT_CAD', 'SELECT_COUNT_CAD']:
                    queries[query_type] = self.materialize_placeholders_with_phrases(analyzed_shape, {'CAR'}, SynonymesGenerator(self.subclass_templates[query_type], self.synonymes).get_all_expansions())

            if not analyzed_shape['complex_range'] and len(analyzed_shape['range_elems']) > 0:
                for query_type in ['SELECT_CAR', 'SELECT_COUNT_CAR']:
                        queries[query_type] = self.materialize_placeholders_with_phrases(analyzed_shape, {'CAD'}, SynonymesGenerator(self.subclass_templates[query_type], self.synonymes).get_all_expansions())
        return queries
...
class SPARQLOWLGenerator:
    ...
    def make_queries(preprocessed_axiom_shape, analyzed_shape):
        ...
        if analyzed_shape['VERB'] is not None:
            ...
            queries['SELECT_COUNT_VERB'] = 'SELECT COUNT ?x WHERE { ' + ask_for_property + ' }'
        ...
        if not analyzed_shape['complex_domain'] and len(analyzed_shape['domain_elems']) > 0:
            ...
            queries['SELECT_COUNT_CAD'] = 'SELECT COUNT ?x WHERE { ' + ask_for_domain + ' }'
        ...
        if not analyzed_shape['complex_range'] and len(analyzed_shape['range_elems']) > 0:
            ...
            queries['SELECT_COUNT_CAR'] = 'SELECT COUNT ?x WHERE { ' + ask_for_range + ' }'
        return queries
